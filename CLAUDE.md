...
Some links are intentionally disabled on Linux because the checked-in files are still macOS-era and haven't been migrated yet — see the commented-out block in `install.sh` (`bash_profile`, `nvim`, `alacritty.toml`). The Omarchy-only configs under `config/` (hypr, starship) are Linux-only and have no macOS counterpart.
...
- **Window manager (Linux)**: Hyprland via Omarchy; the bar, launcher, and idle/lock run in the Omarchy shell (configured in `~/.config/omarchy/shell.json`, not tracked here); no equivalent on macOS
...
- Hyprland is configured in Lua: user overrides go in `config/hypr/{input,looknfeel,monitors,bindings,autostart}.lua`, loaded after Omarchy's defaults. Validate with `hyprctl reload && hyprctl configerrors`. Only `hyprsunset.conf` and `xdph.conf` remain hyprlang
